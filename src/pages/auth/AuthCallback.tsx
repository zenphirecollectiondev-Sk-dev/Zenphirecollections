...
export default function AuthCallback() {
  ...
  // Validate redirect to internal paths only — prevents open redirect
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      console.error('[AuthCallback] OAuth processing timed out. Redirecting to login.');
      navigate('/login?error=Authentication%20failed%20or%20timed%20out.', { replace: true });
    }, 5000);

    // Listen for auth state change — fires once Supabase processes the OAuth tokens from the hash
